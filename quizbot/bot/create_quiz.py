...
db = pymongo.MongoClient("mongodb+srv://jaseelta111:mvW6wA1yX7WhnhPY@cluster0.swxijv6.mongodb.net/quiezz").quizzes
...
async def start(update, _):
    ...
    if update.message.from_user.id in userDict:
        # user is in the middle of a quiz and can't attempt to a second one
        logger.info('[%s] Creation canceled, because the user is in the middle of a creation.',
                    update.message.from_user.username)
        await update.message.reply_text(
            "You're in the middle of a creation 😉 "
...
async def cancel(update, _):
    """
    Cancels the creation of a quiz by deleting the user's entries.
    """
    logger.info('[%s] Creation canceled by user', update.message.from_user.username)

    # Delete user data
    userDict.pop(update.message.from_user.id, None)  # Ensure it doesn't raise KeyError if user is not found
...
async def enter_type(update, _):
    ...
    if update.message.text == "Enter":
        # User doesn't want to add more questions
        # Asks for randomness
        await update.message.reply_text(
            "Should the questions be displayed in random order? 🤔",
            reply_markup=ReplyKeyboardMarkup(
    ...
    # Check if the entered question type is valid
    if update.message.text not in dict_question_types:
        await update.message.reply_text(
            "Sorry, that question type doesn't exist. Please enter a valid type."
        )
        return 'ENTER_TYPE'
...
async def enter_possible_answer(update, _):
    ...
    # Ask for randomness in answer order
    await update.message.reply_text(
        "Should the answers be displayed in random order? 🤔",
        reply_markup=ReplyKeyboardMarkup(
            [['Yes', 'No']], one_time_keyboard=True)
...
async def enter_randomness_question(update, _):
    """
    After entering whether the order of the answers should be random,
    it adds the question to the quiz.
    After that, it asks for the type of next question.
    """
    user_id = update.message.from_user.id
...
async def enter_randomness_quiz(update, _):
    """
    After entering whether the order of the questions should be random,
    it asks if the result of the question should be displayed after the question itself.
    """
    user_id = update.message.from_user.id
...
async def enter_quiz_name(update, context):
    ...
    await update.message.reply_text(
        "Great! 🥳 I saved your new quiz."
        "You can attempt it by the name {}.".format(quizname),
        reply_markup=ReplyKeyboardRemove()
    )
    logger.info('[%s] Quiz saved as "%s"',
                update.message.from_user.username, update.message.text)
